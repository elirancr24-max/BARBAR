/**
 * Israeli holidays import — kept in sync with prisma/seed-holidays.ts.
 * Gregorian dates of major Jewish holidays (source: Hebcal).
 */
...
export async function importIsraeliHolidays(): Promise<{ imported: number; skipped: number }> {
  let imported = 0;
  let skipped = 0;
  for (const h of ISRAELI_HOLIDAYS) {
    const startDate = new Date(`${h.from}T00:00:00.000Z`);
    const endDate = new Date(`${h.to}T23:59:59.000Z`);
